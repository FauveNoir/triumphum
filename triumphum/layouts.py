...
def getListOfAParametterFromAListOfObjects(givenList=None, parameter=None):
    # Cette fonction fait en sorte de retourner tous les paramettres ayant pour nom `parameter`, des items contenus dans la liste `givenList`.
    # Exemple : si `givenList` contient [foo, bar, baz], et que `parameter` donné en entrée est `xyz`, alors la fonction retourne une liste qui serait équivalente à [foo.xyz, bar.xyz, baz.xyz].

    # Cette fonction est particulièrement utile pour foreacher au sein de `listOfBindings` tous les bindings qui y éxistent et en récupérer l’attribut `code`.
...
def makeLayoutsList():
    # Déploiement des layouts éxistants
